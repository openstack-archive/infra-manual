...
* Select 'Branches'
* Enter ``proposed/<series>`` in the 'Branch Name' field, and ``HEAD`` as the
  'Initial Revision', then press 'Create Branch'.
...
Submit Changes in proposed/* to master
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Changes that originate in proposed/* should also be submitted to
master. Use these commands to make an up-to-date topic branch from
master, then cherry-pick changes from proposed/* to be applied to
master::

  git checkout master
  git pull
  git checkout -b mp-to-master
  git cherry-pick <SHA1 or proposed/<series>>
  git review
  git branch -D mp-to-master

``git cherry-pick proposed/$SERIES`` will pick the most recent commit
from proposed/<series> to apply, if you want a different patch, use
the SHA1 of the commit instead.
