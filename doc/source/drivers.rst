...
.. warning::
   ...
   * Make sure you follow `semver <http://semver.org/>`_ rules when picking the
     version. In particular, if there is a change going into this release
     which requires a higher minimum version of a dependency, then the minor
