...
1. Jenkins leaves a comment in the review with links to the log files
   ...
   ``http://logs.openstack.org/<first two of SHA>/<SHA>``. For example,
   if a change is committed with the sha 'deadbeef123456', the logs will
   be found at ``http://logs.openstack.org/de/deadbeef123456``.
