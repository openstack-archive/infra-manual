...
lists of bugs are available at ``https://bugs.launchpad.net/<projectname>``.
...
<https://wiki.openstack.org/wiki/BugFilingRecommendations>`_.
...
*what do i use?*
    ...
    Try `pastebin <http://paste.openstack.org/>`_. It's easy to read and
...
*do I link to that in my bug report?*
    ...
    Try `showterm <http://showterm.io/>`_. It records your console session
    and replays it with all input and output in the exact timing it happened.
    Just reference from your bug description to the *showterm* link.
...
can lead to confusion by contributors. The following are examples of how some
...
back to |tracker-name|. The following options are available::
