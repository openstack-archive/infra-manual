...
3. If the problem is due to non-deterministic behavior already merged,
   and is unrelated to your change, you should do the following to
   help other developers who may be affected by the same issue, and to
   focus attention of QA, CI, and other developers working to fix
  ...
  2. Identify which project(s) are affected, and search for a related
     ...
     not find an existing bug, file a new one (and be sure to include
...
4. To re-run check or gate jobs, leave a comment on the review
   referencing the bug causing the transient failure (not the bug
   you're attempting to fix with your patch) with the form "recheck bug ####".
...
If you need to re-run tests and it does not make sense to include a
bug number (perhaps there is no error or you're updating test results
...
bug". Please only do this if you are certain there is no bug that
