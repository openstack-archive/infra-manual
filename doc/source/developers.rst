...
* If a test job installs packages from PyPI and not via source, a
  dependent change will not have any effect on the testing. The
...
If you backport a change, to another branch, the gerrit change ID
stays the same. If you add a Depends-On for the backported change
change, it now is also dependent on the backported change. This might
be desirable for some changes and a surprise for others.

A change that is dependent on another can be approved before the
dependent change merges. If the repositories share a change queue, it
will merge automatically after the dependent change merged. But if the
repositories do not share a change queue, it will not merge
