...
Accessing Gerrit over HTTP
--------------------------

If you suspect that ssh over non-standards ports might be blocked
(or you need to access the web using https) then you can configure git-review
to use an https endpoint instead of ssh. Keep in mind that you will need to
generate an `HTTP password in Gerrit
<https://review.openstack.org/#/settings/http-password>`_ to use this
connection. You should then configure git-review to use an http scheme
...
be necesary to remove the Gerrit remote from git::
...
this is the HTTP password from Gerrit).
