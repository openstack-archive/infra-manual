...
If your change has a dependency on something outside of a project, such
as a change for another project or some manual setup, you have to
ensure that the change merges at the right time.

For a change depending on a manual step, mark your change with the
...
* To set up a cross-repo dependency, include
  "Depends-On: <gerrit-change-id>" in the footer of your commit
  message. Use the full Change-ID ('I' + 40 characters). A
  patch can also depend on multiple changes as explained in
  :ref:`multiple_changes`.
...
deployments as it means that two components must be upgraded
