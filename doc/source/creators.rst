...
Update your setup.cfg like the following. It should contain the
``compile_catalog``, ``update_catalog``, and ``extract_messages``
sections as well as a ``packages`` entry in the ``files`` section:
...
Update your setup.cfg like the following. It should contain a
``packages`` entry in the ``files`` section:
...
Create file ``babel-django.cfg`` like:
...
Create file ``babel-djangojs.cfg`` like:
