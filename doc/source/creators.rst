...
Note that you need two separate patchsets to set up your new project
for the ``openstack-infra/project-config`` repository:

* First patchset to create the git repository, configure ACLs, and add
  the git repository to the OpenStack CI system, see
  :ref:`add_project_to_master_projects_list` and following sections.
  For official projects, this patchset should also link via
  ``Needed-By`` to a patchset for the ``openstack/governance``
  repository to add the new repository under the project team, see
  :ref:`add-to-governance-repo`.
* Second patchset to add jobs to your project, see
  :ref:`add_jobs_for_project`. This one will fail Zuul internal
  testing until the first patchset is merged, the repository gets
  creates and Zuul reloads its configuration.

The changes described in this section should be submitted together as
two stacked patchsets to the ``openstack-infra/project-config``
repository. Stack these two patchsets on top of each other to make it
...
At this point, you should submit all the changes discussed so far as a
single patchset to gerrit.
...
.. important::
   ...
   separate patchset stacked on top of the previous patchset. Submit
   them together. This second patchset will fail initially, it can
   only pass once the first patchset has merged - and then you need to
