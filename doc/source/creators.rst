...
Every project needs to have an entry in ``zuul/projects.yaml`` containing an
entry for the ``merge-check`` template.

Edit ``zuul/main.yaml`` and add an entry for your project in alphabetical
order:
...
Adding additional jobs can be done in the central repository of in your
new project's ``.zuul.yaml`` file. For more information on addition additional
jobs into your project, see :ref:`in-repo-zuul-jobs`_.
