...
.. note::
   ...
   The infra script consider a project as a Django project when your repository
   name ends with ``-dashboard``, ``-ui`` or ``-horizon``.  Otherwise your
   project will be recognized as a Python project.
...
The infra script for translation setup work as follows:

* The infra script recognizes a project type based on its repository name.
  If the repository name ends with ``-dashboard``, ``-ui`` or ``-horizon``,
  it is treated as a Django project.
  Otherwise it is treated as a Python project.
* If your repository declares multiple python modules in ``packages`` entry
  in ``[files]`` section in ``setup.cfg``, the infra script runs translation
...
exist, it is interpreted that there is no such modules.
