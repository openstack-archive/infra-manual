...
.. sidebar:: Quick Links
   ...
   * `Zuul v3 documentation`__
   * `zuul-jobs documentation`__
   * `openstack-zuul-jobs documentation`__
...
The rest of the jobs will be migrated to as well, but they post-migration
attention will be needed. If you have custom jobs for your project, you or
someone from your project should keep reading this document, and see
:ref:`legacy-job-migration-details`.
...
Zuul starts by loading the configuration in the `project-config`_
repository in the `project-config zuul.d`_ directory.
...
it loads jobs from the `openstack-zuul-jobs`_ repository in the
`openstack-zuul-jobs zuul.d`_ directory which is where we keep most of
...
up and potentially rewrite them to use the new Zuul v3 features.
...
If an old job, or an old project template, has an entry in the
`zuul/mapping.yaml`_ file in `project-config`_, the new name defined in the
file wins.

If an old job does not have a mapping entry, the following rules apply:
...
.. _zuul/mapping.yaml: https://git.openstack.org/cgit/openstack-infra/project-config/tree/zuul/mapping.yaml
