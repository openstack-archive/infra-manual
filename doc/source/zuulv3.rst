...
All of the auto-converted jobs prefixe with ``legacy-`` should be replaced.
...
.. note:: There may be a couple of edge cases they can't handle yet, but they
          should be good to go for most things.
...
* Stop using ``ZUUL_`` env vars, the ``/etc/nodepool`` directory, and the
  ...
  with in ``/home/zuul``, and the source code for the the project that
  triggered the change is in ``src/{{ zuul.project.canonical_name }}``.
